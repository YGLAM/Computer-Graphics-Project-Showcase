/*This file contains the info related to each object
  ...
  var nodes ={
    loadSceneAssets:async function(){
      //first we load up each obj into the array containing the usable assets
        await infos.forEach(function(model_info) {
  ...
  sceneGraph: function(){
    ...
    roomNode = nodeBuilder(assets["room"], program, utils.multiplyMatrices(utils.MakeScaleMatrix(95.0), utils.MakeRotateXYZMatrix(270.0, 90.0, 0.0)));
    roomNode.setParent(roomPositionNode);

    pedestalPositionNode = positionNodeBuilder(utils.MakeTranslateMatrix(-215.0, 0.0, -25.0));
    pedestalNode = nodeBuilder(assets["pedestal"],program,utils.MakeScaleMatrix(6.5));
    ...
    boatNode = nodeBuilder(assets["boat"], program , utils.MakeScaleMatrix(0.25));
...
function nodeBuilder( obj, program , localMatrix){
  ...
  node.drawInfo = {
      ...
      vertexArray:                createObject(obj.objRef),
