...
var nodes ={
  ...
  sceneGraph: async function(){
    room = await retrieveAsset("boat");
    //console.log("is room async?" + room);
    roomPositionNode = positionNodeBuilder(utils.MakeTranslateMatrix(0,0));
    //console.log("is my program empty"+program);
    roomNode = nodeBuilder(room/*assets["room"]*/, program, utils.multiplyMatrices(utils.MakeScaleMatrix(0.01), utils.MakeRotateXYZMatrix(0.0, 0.0, 0.0)));
    ...
    pedestalPositionNode = positionNodeBuilder(utils.MakeTranslateMatrix(0.0, 0.0, 0.0));
    ...
    //console.log(boat);
    boatPositionNode= positionNodeBuilder(utils.MakeTranslateMatrix(0.0, 0.0, 0.0));
...
function nodeBuilder( obj, program , localMatrix){
  ...
  node.drawInfo = {
      ...
      uvLocation:                 gl.getAttribLocation(program, "in_uv"),
  ...
  tl.loadTexture(baseDir+obj.info.tx_src,node);
  console.log(node);
  return node;
}
/*function nodeModelInfo( objRef){
}
function nodeLocationInfo(){
