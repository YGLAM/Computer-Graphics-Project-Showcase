...
var lights = {
    //Associates each entry in the lightLocation with a uniform entry in the fs.glsl
    ...
    //Links each entry in the obj's lightLocation with the parsed attribute in the lights.json file
    //please note that each lightLocation entry is linked with the corresponding
    //fs.glsl uniform entry
...
// converts the string parameters into a float one that will be used in the
// fs.glsl
function typeToFloat(type) {
  ...
  //we extend the [1x3] color vector into a [1x4] color vector
  function extendColor(color) {
      let out = new Float32Array([color[0], color[1], color[2], 1.0]);
      return out;
  }
  //We compute the light direction transformation as seen in E04
  //we are able to use the sub matrix as we do not do any non-uniform scaling
