...
uniform float texture_mix;//substitutes the orenTexture,ToonTexture, etc
uniform vec4  diffuse_color;//substitutes various diffuse colors 
...
vec4 computeLightColor(vec4 color, float pDecay, vec3 pPos, float sDecay, vec3 spotPosition, float sConeIn, float sConeOut, vec3 dir, float lType) {
  if (lType == 0.0) {
    // Direct
    return color;
  } else if (lType == 1.0) {
    // Point
    return color * pow(pointTarget / length(pPos - fs_position), pDecay);
  } else if (lType == 2.0) {
    // Spot
    vec3 spotLightDir = normalize(spotPosition - fs_position);
    float cosAngle = dot(spotLightDir, spotDir);
    return color * pow(spotTarget/length(spotPosition - fs_position), sDecay) * clamp(((cosAngle - sConeOut) / (sConeIn - sConeOut)), 0.0, 1.0);
    } else {
...
void main() {
 ...
 vec4 diffuse_color = diffuse_color * (1.0 - texture_mix)+ textureColor * texture_mix;
 vec4 ambient_material = ambientColor * (1.0- texture_mix) + textureColor * texture_mix;


 vec3 lightDir = computeLightDir(finalSpotPosition, pointPos, dirDirection, lightType);
 vec4 lightCol = computeLightColor(src_color, pointDecay, pointPos, spotDecay, finalSpotPosition, spotConeIn/100.0, spotConeOut, lightDir, lightType);

 vec4 diffuse = computeDiffuse(lightDir, lightCol, nNormal, diffuse_color, nEyeDirection, lightDiffuseType);

 vec4 ambient = computeAmbient(ambient_material, nNormal, lightAmbientType);
