...
vec4 computeDiffuse(vec3 lightDirection, vec4 lightColor, vec3 nVec, vec4 diffLColor, vec4 diffTColor, vec3 eyeVec, float lightDiffuseType) {
    float dotN = clamp(dot(nVec, lightDirection), 0.0, 1.0);
  if (lightDiffuseType == 0.0) {
    vec4 col = lightColor * diffLColor;
    // Lambert
    return dotN * col;
  } else if (lightDiffuseType == 1.0) {
    // Toon
...
void main() {
 ...
 vec4 diffuse = computeDiffuse(lightDir, lightCol, nNormal, diffLColor, diffTColor, nEyeDirection, lightDiffuseType);
